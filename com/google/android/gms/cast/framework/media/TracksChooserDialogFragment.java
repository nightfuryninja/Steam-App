...
public class TracksChooserDialogFragment extends DialogFragment {
    ...
    class C02411 implements OnClickListener {
        ...
        C02411(TracksChooserDialogFragment tracksChooserDialogFragment) {
    ...
    private void zza(View view, zzf com_google_android_gms_cast_framework_media_zzf, zzf com_google_android_gms_cast_framework_media_zzf2) {
        ListView listView = (ListView) view.findViewById(C0154R.id.text_list_view);
        ListView listView2 = (ListView) view.findViewById(C0154R.id.audio_list_view);
        TextView textView = (TextView) view.findViewById(C0154R.id.text_empty_message);
        TextView textView2 = (TextView) view.findViewById(C0154R.id.audio_empty_message);
        ...
        TabHost tabHost = (TabHost) view.findViewById(C0154R.id.tab_host);
        tabHost.setup();
        TabSpec newTabSpec = tabHost.newTabSpec("textTab");
        if (com_google_android_gms_cast_framework_media_zzf.getCount() == 0) {
            listView.setVisibility(4);
            newTabSpec.setContent(C0154R.id.text_empty_message);
        } else {
            textView.setVisibility(4);
            newTabSpec.setContent(C0154R.id.text_list_view);
        }
        newTabSpec.setIndicator(getActivity().getString(C0154R.string.cast_tracks_chooser_dialog_subtitles));
        tabHost.addTab(newTabSpec);
        TabSpec newTabSpec2 = tabHost.newTabSpec("audioTab");
        if (com_google_android_gms_cast_framework_media_zzf2.getCount() == 0) {
            listView2.setVisibility(4);
            newTabSpec2.setContent(C0154R.id.audio_empty_message);
        } else {
            textView2.setVisibility(4);
            newTabSpec2.setContent(C0154R.id.audio_list_view);
        }
        newTabSpec2.setIndicator(getActivity().getString(C0154R.string.cast_tracks_chooser_dialog_audio));
    ...
    private MediaTrack zztm() {
        return new Builder(-1, 1).setName(getActivity().getString(C0154R.string.cast_tracks_chooser_dialog_none)).setSubtype(2).setContentId("").build();
    }
    ...
    public Dialog onCreateDialog(Bundle bundle) {
        ...
        View inflate = getActivity().getLayoutInflater().inflate(C0154R.layout.cast_tracks_chooser_dialog_layout, null);
        zza(inflate, com_google_android_gms_cast_framework_media_zzf, com_google_android_gms_cast_framework_media_zzf2);
        builder.setView(inflate).setPositiveButton(getActivity().getString(C0154R.string.cast_tracks_chooser_dialog_ok), new OnClickListener(this) {
            final /* synthetic */ TracksChooserDialogFragment zzarP;

            public void onClick(DialogInterface dialogInterface, int i) {
                this.zzarP.zza(com_google_android_gms_cast_framework_media_zzf, com_google_android_gms_cast_framework_media_zzf2);
            }
        }).setNegativeButton(C0154R.string.cast_tracks_chooser_dialog_cancel, new C02411(this));
        if (this.mDialog != null) {
            this.mDialog.cancel();
            this.mDialog = null;
        }
