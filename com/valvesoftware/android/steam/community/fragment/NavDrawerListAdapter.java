...
public class NavDrawerListAdapter extends BaseExpandableListAdapter {
    ...
    public View getGroupView(final int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        if (v == null) {
            v = this.layoutInflater.inflate(C0815R.layout.nav_group_list_item, null);
        } else {
            NavDrawerItem old = (NavDrawerItem) v.getTag();
            if (old != null) {
                old.setView(null);
            }
            View navNotificationCtn = v.findViewById(C0815R.id.nav_item_notification_count_ctn);
        ...
        v.setTag(current);
        current.setView(v);
        TextView navItemName = (TextView) v.findViewById(C0815R.id.nav_item_name);
        ImageView expandCollapseIcon = (ImageView) v.findViewById(C0815R.id.group_expand_collapse_icon);
        ...
        if (current.hasChildren()) {
            if (isExpanded) {
                expandCollapseIcon.setImageResource(C0815R.drawable.ic_action_expand_less);
            } else {
                expandCollapseIcon.setImageResource(C0815R.drawable.ic_action_expand_more);
            }
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        if (v == null) {
            v = this.layoutInflater.inflate(C0815R.layout.nav_list_item, null);
        } else {
            NavDrawerItem old = (NavDrawerItem) v.getTag();
            if (old != null) {
                old.setView(null);
